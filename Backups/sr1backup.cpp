...
mainMenuStr, razielMenuStr, holdSRvrStr, specRvrStr, mateRvrStr, fireRvrStr, cameraMenuStr, menuState,
lockRaziel, objectAccess, FX_StartGenericParticle, FX_StartGenericBlastRing, CollideReaverProjectile,
...
void betaSpiral() {
	...
	SR1.WriteXString(allocated2, "83 3D 50 88 B0 00 01 74 07 68 00 40 00 00 EB 05 68 00 00 40 00 E9");
...
void infoFix() {		// Code seems the work once in the game.  Trainer crashes due to flaw in WriteXString :(
	if (!allocated2) {
		allocated2 = SR1.Alloc(128);
	}
	// Keep Health/Mana Levels up to date.
	DWORD next = allocated2;
	SR1.WriteXString(next+=0x1A, "90 90 8A 05 10 88 B0 00");
	SR1.WriteXString(next+=0x08, "88 05 4C 5F 4F 00");
	SR1.WriteXString(next+=0x06, "8A 05 2A 88 B0 00");
	SR1.WriteXString(next+=0x06, "C0 E8 02");
	SR1.WriteXString(next+=0x03, "88 05 50 5F 4F 00 C3 90 90 90");

	SR1.Write(0x00492840-0x05, (BYTE) 0xE8);
	SR1.Write(0x00492840-0x04, (allocated2+0x1C)-0x00492840);	// There it was!
	SR1.Write(0x00491FBB, (0x00492840-0x05)-0x00491FBB-0x04);
	SR1.Write(0x004923AC, (0x00492840-0x05)-0x004923AC-0x04);
	
	// Allow for invincible working.
	/*BYTE* zeros = new BYTE[44];
	memset(zeros, 0x90, 29);
	SR1.Write(0x00492AF6, (BYTE) 0xE8);
	SR1.Write(0x00492AF7, (next+0x09)-0x00492AFB);
	SR1.Write(0x00492AFB, zeros, 29);
	SR1.WriteXString(next+=0x09, "F6 40 08 01 8B 50 10 8B 48 14 8B 02 74 08 21 C8 3B C1 74 08 EB 0D 21 C8 3B C1 74 07 68 B4 8D 4F 00 EB 05 68 B8 8D 4F 00 E8 29 E2 A3 FB 83 C4 04 C3 90 90");
	//SR1.Write(next+=0x25, 0x0049e290-(next+4));
	//SR1.WriteXString(next+=4, "83 C4 04 C3 90 90");

	SR1.Write(0x004f6698, 0x00000001);
	SR1.Write(0x004f66a0, 0x00C66DE4);
	SR1.Write(0x004f66a4, 0x00000800);*/

	//	[REALLY KEEP INFO BETWEEN LOADS]
	SR1.WriteXString(next+=0x33, "FF 35 F4 44 C6 00 FF 35 F8 44 C6 00 FF 35 FC 44 C6 00 FF 35 E0 6D C6 00 FF 35 E4 6D C6 00 FF 35 10 88 B0 00 FF 35 28 88 B0 00 B9 80 0A 44 00 56 FF D1 5E F7 05 64 6E C6 00 00 00 20 00 75 04 83 C4 1C C3 8F 05 28 88 B0 00 8F 05 10 88 B0 00 8F 05 E4 6D C6 00 8F 05 E0 6D C6 00 8F 05 FC 44 C6 00 8F 05 F8 44 C6 00 8F 05 F4 44 C6 00 C3");
	SR1.Write(0x004566bb, next-0x004566bf);
	...
	*///SR1.WriteXString(0x00438E51, "90 90 81 25 64 6E C6 00 FF FF DF FF");	// Same again but without loading condition
	...
	SR1.Write(0x0043A025, (WORD) 0x3874);										// Same as above...
	SR1.WriteXString(0x004C6D08, "81 E6 FF FF DF FF");
	//SR1.Write(0x0043A053, (WORD)0x0AEB);	// Always skip switch off

	// [NEW TOGGLE BIT CONTROL]
	SR1.WriteXString(0x0049268B, "59 09 08 5E C3");
	SR1.WriteXString(0x004926AF, "51 23 08 3B 0C 24 75 D4 59 F7 D1 21 08 5E C3 56 E8 6C 7E FE FF 83 C4 04 85 C0 75 DA 5E C3 90 90 90");
	SR1.Write(0x004926A3, (WORD) 0x1974);

	SR1.Write(0x004F5BB4, (BYTE) 0xD2);		// Clearer Curser
}
